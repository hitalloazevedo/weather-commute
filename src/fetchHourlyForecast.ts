...
export async function fetchHourlyForecast(locations:Array<LocationType>):Promise<Array<ForecastType>>{
 return Promise.all(locations.map(async location=>{
   ...
   const now=new Date();
   const list=data.hourly.time.map((t:string,i:number)=>({t,i}))
      .filter((x:any)=>new Date(x.t)>=now)
      .slice(0,7)
      .map((x:any)=>({
        dt:Math.floor(new Date(data.hourly.time[x.i]).getTime()/1000),
        main:{temp:data.hourly.temperature_2m[x.i],feels_like:0,temp_min:0,temp_max:0,pressure:0,sea_level:0,grnd_level:0,humidity:0,temp_kf:0},
        weather:[{id:data.hourly.weather_code[x.i],main:"",description:WEATHER_CODES[data.hourly.weather_code[x.i]]??"Unknown",icon:""}],
        clouds:{all:0},wind:{speed:0,deg:0,gust:0},visibility:0,pop:0,sys:{pod:""},dt_txt:data.hourly.time[x.i]
      }));
